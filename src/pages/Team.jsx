    ...
    <>
      <div className="max-w-7xl mx-auto py-16">
        <div className="max-w-6xl mx-auto text-center">
          <h2 className="text-4xl font-bold text-orange-400 font-domine">
            Meet our Team Members
          </h2>
          <p className="text-gray-600 mt-4 px-1 ">
            Dedicated professionals working together to bring success.
          </p>
        </div>
        ...
        <div className="mt-12 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 max-w-8xl mx-auto px-6">
          {filteredMembers.map((member, index) => (
            <div key={index} className="rounded-lg p-4 sm:p-6 text-center ">
              <div className="aspect-square w-full max-w-xs mx-auto">
                <img
                  src={member.image}
                  alt={member.name}
                  className="w-full h-full rounded-full object-center transition-transform duration-300 hover:scale-110"
                />
              </div>
              <h3 className="text-lg sm:text-xl font-domine font-semibold mt-4 text-orange-400">
                {member.name}
              </h3>
              <p className="text-sm font-semibold sm:text-base text-gray-600">
                {member.role}
              </p>
